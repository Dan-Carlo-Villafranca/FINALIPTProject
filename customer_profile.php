...
$success_msg = isset($_GET['success']) ? "Your profile has been updated!" : "";
$error_msg = "";
    ...
    try {
        ...
        if (isset($_POST['update_account'])) {
            ...
            if (password_verify($curr_pass, $user['password'])) {
                ...
                $error_msg = "Incorrect current password. Changes not saved.";
            }
        }
...
<head>
    <meta charset="UTF-8">
    <title>Rent N' Ride | My Profile</title>
    ...
    <style>
        ...
        .nav-link { display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; border-radius: 0.75rem; color: #94a3b8; transition: all 0.2s; }
        .nav-link:hover { background: #1e293b; color: #fff; }
        .nav-link.active { background: #4f46e5; color: #fff; box-shadow: 0 10px 15px -3px rgba(79, 70, 229, 0.2); }
    </style>
</head>
<body class="bg-slate-50" x-data="{ showLogoutModal: false, showAccountModal: false }">
    ...
    <div class="flex min-h-screen">
        <aside class="w-64 glass-sidebar text-white flex flex-col fixed h-full">
            ...
            <nav class="flex-1 px-4 space-y-2">
                <a href="customerdashboard.php" class="nav-link <?= $current_page == 'customer_dashboard.php' ? 'active' : '' ?>">
                    <i class="fa-solid fa-house w-5"></i> <span>Home</span>
                </a>
                <a href="#" class="nav-link">
                    <i class="fa-solid fa-calendar-check w-5"></i> <span>My Bookings</span>
                </a>
                <hr class="border-slate-800 my-4 mx-2">
                <a href="customer_profile.php" class="nav-link <?= $current_page == 'customer_profile.php' ? 'active' : '' ?>">
            ...
            <div class="p-4 border-t border-slate-800">
                <div class="flex items-center space-x-3 mb-4 px-2">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-indigo-400 font-bold uppercase tracking-wider"><?= $user['role'] ?? 'Customer' ?></p>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div class="lg:col-span-1">
                            <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 text-center">
                                ...
                                <h2 class="text-xl font-bold text-slate-800 mt-2"><?= htmlspecialchars($user['display_name'] ?: $user['username']) ?></h2>
                                <p class="text-xs text-slate-400 uppercase font-bold tracking-widest mt-1"><?= $user['role'] ?? 'Customer' ?></p>
                            </div>
                        </div>
    ...
    <div x-show="showAccountModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="relative bg-white rounded-[2rem] max-w-md w-full p-8 shadow-2xl" x-transition x-data="{ showPass: false, showNewPass: false }">
            ...
            <p class="text-slate-500 text-sm mb-6">Update your login credentials.</p>
            <form method="POST" class="space-y-4">
                ...
                <div class="relative">
                    ...
                    <input :type="showPass ? 'text' : 'password'" name="current_password" class="profile-input" placeholder="Confirm to save" required>
                ...
                <div class="relative">
                    ...
                    <input :type="showNewPass ? 'text' : 'password'" name="new_password" class="profile-input" placeholder="Leave blank to keep current">
    ...
    <div x-show="showLogoutModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="relative bg-white rounded-[2rem] max-w-sm w-full p-8 shadow-2xl text-center">
            ...
            <p class="text-slate-500 mb-8">Are you sure you want to leave?</p>
