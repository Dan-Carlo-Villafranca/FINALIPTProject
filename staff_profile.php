...
// Force no-cache headers to prevent back-button UX issues
...
// 1. Fetch current data BEFORE processing post to handle partial updates
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if (isset($_POST['update_profile'])) {
            // Using shorthand: If input is empty, use the current value from $user
            $display_name = !empty(trim($_POST['display_name'])) ? trim($_POST['display_name']) : ($user['display_name'] ?? '');
            $first_name   = !empty(trim($_POST['first_name']))   ? trim($_POST['first_name'])   : ($user['first_name'] ?? '');
            $middle_name  = !empty(trim($_POST['middle_name']))  ? trim($_POST['middle_name'])  : ($user['middle_name'] ?? '');
            $last_name    = !empty(trim($_POST['last_name']))    ? trim($_POST['last_name'])    : ($user['last_name'] ?? '');
            $birthday     = !empty($_POST['birthday'])           ? $_POST['birthday']           : ($user['birthday'] ?? '');
            $email        = !empty(trim($_POST['email']))        ? trim($_POST['email'])        : ($user['email'] ?? '');
            $address      = !empty(trim($_POST['address']))      ? trim($_POST['address'])      : ($user['address'] ?? '');
            $contact      = !empty(trim($_POST['contact_number'])) ? trim($_POST['contact_number']) : ($user['contact_number'] ?? '');
            $gender       = !empty($_POST['gender'])             ? $_POST['gender']             : ($user['gender'] ?? '');
        ...
        // --- HANDLE ACCOUNT SECURITY UPDATE ---
        if (isset($_POST['update_account'])) {
            ...
            if (password_verify($curr_pass, $user['password'])) {
                ...
                if (!empty($new_pass)) {
                    ...
                    $update_params[] = password_hash($new_pass, PASSWORD_DEFAULT);
                }
                ...
                if (!empty($update_fields)) {
                    ...
                    $success_msg = "Account credentials updated!";
                ...
                $error_msg = "Current password incorrect. No changes saved.";
            }
        }
...
<head>
    <meta charset="UTF-8">
    <title>Rent N' Ride | Staff Profile</title>
...
<body class="bg-slate-50" x-data="{ showLogoutModal: false, showAccountModal: false }">
        ...
        <aside class="w-64 glass-sidebar text-white flex flex-col fixed h-full">
            ...
            <div class="p-4 border-t border-slate-800">
                <div class="flex items-center space-x-3 mb-4 px-2">
                    ...
                    <div>
                        <p class="text-sm font-semibold truncate w-32"><?= htmlspecialchars($user['username'] ?? ''); ?></p>
        ...
        <main class="flex-1 ml-64 p-8">
            <div class="max-w-4xl mx-auto">
                <div class="flex justify-between items-end mb-8">
                    <div>
                        <h1 class="text-3xl font-bold text-slate-800">Staff Profile</h1>
                        <p class="text-slate-500">Manage your account and personal info.</p>
                    </div>
                    <button @click="showAccountModal = true" class="px-6 py-3 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition flex items-center shadow-sm">
                        <i class="fa-solid fa-shield-halved mr-2 text-indigo-500"></i> Edit Account Details
                    </button>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div class="lg:col-span-1">
                            <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 text-center">
                                ...
                                <h2 class="text-xl font-bold text-slate-800 mt-2"><?= htmlspecialchars(($user['display_name'] ?? $user['username']) ?? 'User') ?></h2>
                                <p class="text-xs text-slate-400 uppercase font-bold tracking-widest mt-1"><?= $role ?></p>
                            </div>
                        </div>
    ...
    <div x-show="showAccountModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="relative bg-white rounded-[2rem] max-w-md w-full p-8 shadow-2xl" x-transition x-data="{ showPass: false, showNewPass: false }">
            ...
            <p class="text-slate-500 text-sm mb-6">Update your login information.</p>

            <form method="POST" class="space-y-4">
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase mb-2">Username</label>
                    <input type="text" name="new_username" class="profile-input bg-slate-50" value="<?= htmlspecialchars($user['username'] ?? '') ?>">
                </div>
                <hr class="my-4 border-slate-100">
                <div class="relative">
                    <label class="block text-xs font-bold text-slate-400 uppercase mb-2">Current Password</label>
                    <input :type="showPass ? 'text' : 'password'" name="current_password" class="profile-input" placeholder="Verify current password" required>
                ...
                <div class="flex gap-3 pt-4">
                    <button type="submit" name="update_account" class="flex-1 py-4 bg-indigo-600 text-white font-bold rounded-2xl shadow-lg">Save Changes</button>
